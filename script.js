...
/*
Отличная работа! Код читаемый, хорошо оргинизован.
Вы верно используете инкапсуляцию внутри классов. Теперь ваш код легче поддерживать и расширять.

Можно лучше:

1. Во время создания карточки не хорошо навешивать обработчик события на каждый интерактивный элемент
попробуйте для себя сделать тот же функционал, но используя делегирование через event.target

2. Использование числительных в именах переменных не очень хорошая практика ( submit1 ). Это вызовет проблемы при дальнейшем развитии проекта, возможно
вам в будущем придется заменять формы или добавлять новые, а может переиспользовать код.
 */
